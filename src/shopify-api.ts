import { type HTTPMethod, STATUS_CODE } from "./http.ts";
...
export class ShopifyAPI {
   ...
   async getAccessToken(
      ...
      return await this.request("/admin/oauth/access_token", "POST", {
         client_id,
         client_secret,
         code,
      });
   ...
   async getShop() {
      return (await this.request(`/admin/api/${this.#apiVersion}/shop.json`))
         .shop;
   }

   async getWebhooks() {
      return (
         await this.request(`/admin/api/${this.#apiVersion}/webhooks.json`)
      ).webhooks as Array<Webhook>;
   }
   ...
   async createWebhook(webhook: Webhook) {
      ...
      return (
         await this.request(
            `/admin/api/${this.#apiVersion}/webhooks.json`,
            "POST",
            { webhook },
         )
      ).webhook as Webhook;
   ...
   async updateWebhook(webhook: Webhook) {
      ...
      return (
         await this.request(
            `/admin/api/${this.#apiVersion}/webhooks/${webhook.id!}.json`,
            "PUT",
            { webhook },
         )
      ).webhook as Webhook;
   ...
   async removeWebhook(webhookID: string | number) {
      return await this.request(
         `/admin/api/${this.#apiVersion}/webhooks/${webhookID}.json`,
         "DELETE",
      );
   }
   ...
   async getScriptTags() {
      return (
         await this.request(`/admin/api/${this.#apiVersion}/script_tags.json`)
      ).script_tags as Array<ScriptTag>;
   }
   ...
   async updateScriptTag(script_tag: ScriptTag) {
      ...
      return (
         await this.request(
            `/admin/api/${this.#apiVersion}/script_tags/${script_tag.id}.json`,
            "PUT",
            { script_tag },
         )
      ).script_tag as ScriptTag;
   ...
   async removeScriptTag(script_tag_id: string | number) {
      return await this.request(
         `/admin/api/${this.#apiVersion}/script_tags/${script_tag_id}.json`,
         "DELETE",
      );
   }
   ...
   async getOrders(params?: OrderParameters) {
      ...
      return (
         await this.request(
            `/admin/api/${this.#apiVersion}/orders.json${query && `?${query}`}`,
         )
   ...
   async getProducts() {
      return (
         await this.request(`/admin/api/${this.#apiVersion}/products.json`)
   ...
   async getRecurringApplicationCharges() {
      ...
      ).recurring_application_charges as Array<RecurringApplicationCharge>;
   }
   ...
   async createRecurringApplicationCharge(
      ...
      recurring_application_charge = {
         trial_days: 0,
         return_url: `https://${this.#shop}/admin/apps/${apiKey}`,
         ...recurring_application_charge,
      };
      return (
         await this.request(
